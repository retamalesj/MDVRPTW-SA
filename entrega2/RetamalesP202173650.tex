...
Posteriormente, se expone su modelo matemático de programación entera mixta, se discuten tendencias actuales de investigación y trabajo futuro.
...
Debido a su complejidad computacional y a su naturaleza NP-hard, hace que encontrar soluciones eficientes siga siendo un desafío importante dentro del área de optimización
combinatoria.
...
\begin{itemize}
  \item $x_{ijk} \in \{0,1\}$: indica si el vehículo $k$ viaja del nodo $i$ al nodo $j$.
  \item $t_{ik}$: tiempo de inicio de servicio del vehículo $k$ en el nodo $i$.
  \item $w_{ik}$: penalización por violación de ventana de tiempo.
\end{itemize}

\subsection{Restricciones}

Las restricciones fundamentales del MDVRP-TW son:
\begin{itemize}
  \item Cada cliente $c_i \in C$ debe ser visitado exactamente una vez.
  \item Todos los vehículos parten en tiempo $t = 0$ desde su depósito asignado.
  \item La suma de demandas en cada ruta no puede exceder la capacidad $Q$ del vehículo.
  \item Los vehículos deben retornar al depósito de origen dentro de los intervalos de tiempo especificados, con penalizaciones en sus desvíos.
  \item Cuando un vehículo llega al cliente $i$ en tiempo $t$: si $t < e_i$, el vehículo debe esperar; si $e_i \leq t \leq l_i$, inicia el servicio inmediatamente; si $t > l_i$, se aplica una penalización lineal proporcional a $(t - l_i)$.
\end{itemize}
